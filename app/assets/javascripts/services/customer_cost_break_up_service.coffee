  ...
  costBreakUpInstance.subTotal = (trip)->
    ...
    if trip.flight_plan
      for flight_plan in trip.flight_plan
        cost += flight_plan.flight_cost + (costBreakUpInstance.commission/100 * flight_plan.flight_cost)
        cost += flight_plan.handling_cost_at_takeoff + (costBreakUpInstance.commission/100 * flight_plan.handling_cost_at_takeoff)
        cost += flight_plan.landing_cost_at_arrival + (costBreakUpInstance.commission/100 * flight_plan.landing_cost_at_arrival)
        ...
        if flight_plan.chosen_intermediate_plan
          ...
          if chosen_plan and flight_plan.chosen_intermediate_plan == 'empty_leg_plan'
            for empty_leg in chosen_plan
              cost += empty_leg.flight_cost + (costBreakUpInstance.commission/100 * empty_leg.flight_cost)
              cost += empty_leg.handling_cost_at_takeoff + (costBreakUpInstance.commission/100 * empty_leg.handling_cost_at_takeoff)
              cost += empty_leg.landing_cost_at_arrival + (costBreakUpInstance.commission/100 * empty_leg.landing_cost_at_arrival)
          ...
          if chosen_plan and flight_plan.chosen_intermediate_plan == 'accommodation_plan'
            cost += chosen_plan.cost + (costBreakUpInstance.commission/100 * chosen_plan.cost)
    else
      for activity in trip.activities
        cost += activity.flight_cost + (costBreakUpInstance.commission/100 * activity.flight_cost)
        cost += activity.handling_cost_at_takeoff + (costBreakUpInstance.commission/100 * activity.handling_cost_at_takeoff)
        cost += activity.landing_cost_at_arrival + (costBreakUpInstance.commission/100 * activity.landing_cost_at_arrival)
        ...
        if activity.accommodation_plan and activity.accommodation_plan.cost
          cost += activity.accommodation_plan.cost + (costBreakUpInstance.commission/100 * activity.accommodation_plan.cost)
    cost
